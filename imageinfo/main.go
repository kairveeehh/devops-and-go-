...
func analyze(cli *client.Client, imageID string) {
	...
	if err != nil {
		// Using a simple empty struct for options
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "Maximum supported API version is") {
				color.Yellow("Use the -sV flag to change your client version. ./executable -sV=1.36 %s", imageID)
			}
			return
		}
		defer out.Close()
...
func analyzeImageFilesystem(cli *client.Client, imageID string) error {
	...
	for {
		...
		if imageFile.Name == "manifest.json" {
...
func main() {
	...
	if len(*filelist) > 0 {
		...
		color.Red("Please provide a repository image to analyze. ./executable nginx:latest")
		return
	}
	cli.Close()
}
